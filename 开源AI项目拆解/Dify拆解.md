...
指定分段标识符、最大长度、重叠长度后，切块器按以下逻辑运行：遇到分段标识符时强制切分；累积长度超过最大长度时切分；重叠长度保证相邻 Chunk 有公共文本，防止跨块语义断裂。开启 Q&A 分段模式后，会调用大模型基于通用分块结果生成可能对应的问题，形成"问题-答案对"存储——这样检索时用问题匹配问题，语义相似度远高于用问题匹配原文段落。

v1.1.0 引入的元数据过滤功能允许为文档打标签（字符串、数字、时间三种类型），在检索时可以先按元数据过滤缩小范围，再做语义检索，大幅提升检索效率和精度。适合多部门、多产品线文档混存的企业场景——可以设置"只检索市场部文档"或"只检索2025年后更新的内容"。
